 ...
 *
 * @param cfg configuration struct; the caller should ensure cfg is
 *            valid before calling this function.
 * @return 0 on success, negative error code on failure. The caller
 *         would need to check the return value.
 * @warning This function could block if the event loop is busy.
 * @thread_safety Not safe to call concurrently.
 * @ownership Caller owns the returned handle.
 */
